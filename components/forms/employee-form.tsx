...
type EmployeFormValues = z.infer<typeof formSchema>;

interface EmployeFormProps {
  initialData: any | null;
}

export const EmployeForm: React.FC<EmployeFormProps> = ({ initialData }) => {
  const router = useRouter();
  const { toast } = useToast();
  const [loading, setLoading] = useState(false);

  const title = initialData ? 'Edit Broker' : 'Add Buyer';
